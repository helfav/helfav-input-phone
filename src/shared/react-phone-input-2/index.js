...
class PhoneInput extends React.PureComponent {
  ...
  updateFormattedNumber(value) {
    ...
    if (!startsWith(inputNumber, '+')) {
      ...
      const dialCode =
        newSelectedCountry && !startsWith(inputNumber.replace(/\D/g, ''), newSelectedCountry.dialCode)
            ...
            searchedCountries.map((country, index) => {
              if (country.isShow) {
                if (this.state.selectedCountry) {
                  if (!highlightCountryIndex && this.state.selectedCountry.iso2 == country.iso2) {
                    highlightCountryIndex = index
                    this.scrollTo(this.getElement(index))
                  }
                }
              }
            })
      ...
      this.setState(
        {
          ...
          highlightCountryIndex: this.props.disableAreaCodes
            ? onlyCountries.findIndex(o => o.iso2 == this.state.selectedCountry.iso2)
  ...
  handleInput = e => {
    ...
    if (!this.props.isValid(formattedNumber.replace(/\D/g, ''))) {
      formattedNumber = ''
    }

    let array = formattedNumber.split('')

    let done = false
    for (let i = newSelectedCountry.dialCode.length + 1; !done && i < array.length; i++) {
  ...
  handleKeydown = e => {
    ...
    const moveHighlight = direction => {
      this.setState(
        ...
        () => {
          this.scrollTo(this.getElement(this.state.highlightCountryIndex + this.state.preferredCountries.length), true)
        }
      )
    }
  ...
  getCountryDropdownList = () => {
    let { preferredCountries, showDropdown, searchValue } = this.state
    ...
    let countryDropdownList = searchedCountries.map((country, index) => {
      if (country.isShow) {
        let highlightCountryIndex = 0
        if (this.state.selectedCountry) {
          if (this.state.selectedCountry.iso2 == country.iso2) {
            highlightCountryIndex = index
          }
        }
    ...
    return (
      <ul ref={el => (this.dropdownRef = el)} className={dropDownClasses} style={this.props.dropdownStyle}>
        {enableSearchField && (
          <li
  ...
  render() {
    ...
    console.log('this.props', this.props, this.props.isValid, !this.props.isValid(formattedNumber.replace(/\D/g, '')))
    ...
    return (
      <div className={this.props.containerClass} style={this.props.containerStyle} onKeyDown={this.handleKeydown}>
        <input
